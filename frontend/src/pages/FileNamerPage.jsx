...
async function downloadZip(jobId, renameMap) {
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'renamed_files.zip'; a.click()
  URL.revokeObjectURL(url)
}

export default function FileNamerPage() {
  ...
  async function handlePropose() {
    ...
    setRunning(true); setError(''); setProposals([]); setEdits({}); setJobId(null)
    try {
      const data = await proposeRenames(files)
      setProposals(data.proposals); setJobId(data.job_id)
  ...
  async function handleDownload() {
    ...
    try { await downloadZip(jobId, renameMap) } catch(e) { setError(e.message) }
  }
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 24px 80px', display: 'flex', flexDirection: 'column', gap: 24 }}>

      <div className="card">
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--ox)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>01 — Upload Files</div>
          <p style={{ color: 'var(--muted)', fontSize: 13 }}>Drop vendor statement PDFs or Excel files to rename</p>
        </div>
      ...
      {proposals.length > 0 && (
        <div ref={resultsRef} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {proposals.map((p, i) => {
            const row = getRow(i)
            return (
              <div key={i} className="card" style={{ borderColor: p.multi ? 'rgba(252,211,77,0.3)' : 'var(--border)', padding:'16px 20px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
                  <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--muted)' }}>{p.original}</span>
                  {p.multi && (
                    <span style={{ fontFamily:'var(--mono)', fontSize:9, color:'#FCD34D', background:'rgba(252,211,77,0.1)',
                      border:'1px solid rgba(252,211,77,0.3)', borderRadius:2, padding:'1px 6px', letterSpacing:'0.1em', textTransform:'uppercase' }}>
                      Multi-entity split
                    </span>
                  )}
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:8, marginBottom:12 }}>
                  <div>
                    ...
                    <input value={row.entity} onChange={e => updateEdit(i,'entity',e.target.value)} style={inputStyle} />
                  </div>
                  <div>
                    ...
                    <input value={row.vendor} onChange={e => updateEdit(i,'vendor',e.target.value)} style={inputStyle} />
                  </div>
                  <div>
                    ...
                    <input value={row.date} onChange={e => updateEdit(i,'date',e.target.value)} style={{ ...inputStyle, width:80 }} />
                  </div>
                </div>
                <div style={{ fontFamily:'var(--mono)', fontSize:12, color:'var(--ox)', background:'rgba(255,112,48,0.06)',
                  border:'1px solid var(--ox-b)', borderRadius:2, padding:'6px 10px' }}>
                  → {buildProposed(i)}
                </div>
              </div>
            )
          ...
          <button className="btn btn-primary" onClick={handleDownload} style={{ fontSize:15, padding:'16px 24px', marginTop:8 }}>
